...
LOG = logging.getLogger('ryu.ofproto.opp_v1_0_parser')
...
def OFPExpActionSetState(state, table_id, bit=0, hard_timeout=0, idle_timeout=0, hard_rollback=0, idle_rollback=0, state_mask=0xffffffff, fields=[]):
    ...
    if field_count > oppproto.MAX_FIELD_COUNT:
        field_count = 0
        LOG.debug("OFPExpActionSetState: Number of fields given > MAX_FIELD_COUNT")

    act_type=oppproto.OFPAT_EXP_SET_STATE
    data=struct.pack(oppproto.OFP_EXP_ACTION_SET_STATE_PACK_STR, act_type, state, state_mask, table_id, hard_rollback, idle_rollback, hard_timeout*1000, idle_timeout*1000, bit, field_count)
...
def OFPExpActionSetGlobalState(global_state, global_state_mask=0xffffffff):
    ...
    ================ ======================================================
    """
    act_type=oppproto.OFPAT_EXP_SET_GLOBAL_STATE
    data=struct.pack(oppproto.OFP_EXP_ACTION_SET_GLOBAL_STATE_PACK_STR, act_type, global_state, global_state_mask)
    return ofproto_parser.OFPActionExperimenterUnknown(experimenter=0XBEBABEBA, data=data)
...
def OFPExpActionIncState(table_id):
    ...
    table_id         Stage ID
    """
    act_type=oppproto.OFPAT_EXP_INC_STATE
    data=struct.pack(oppproto.OFP_EXP_ACTION_INC_STATE_PACK_STR, act_type, table_id)
...
def OFPExpActionSetDataVariable(table_id, opcode, bit=0, output_gd_id=None, output_fd_id=None, operand_1_fd_id=None, operand_1_gd_id=None, operand_1_hf_id=None, operand_2_fd_id=None, operand_2_gd_id=None, operand_2_hf_id=None, operand_2_cost=None, operand_3_fd_id=None, operand_3_gd_id=None, operand_3_hf_id=None,operand_4_fd_id=None, operand_4_gd_id=None, operand_4_hf_id=None, coeff_1=0, coeff_2=0, coeff_3=0, coeff_4=0, fields=[]):
    ...
    act_type=oppproto.OFPAT_EXP_SET_DATA_VAR

    if opcode>oppproto.OPCODE_POLY_SUM:
    ...
    if operand_1_fd_id!=None:
        if operand_1_fd_id<0 or operand_1_fd_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid flow data variable ID")
        operand_types=oppproto.OPERAND_TYPE_FLOW_DATA_VAR<<14
        operand_1=operand_1_fd_id
    elif operand_1_gd_id!=None:
        if operand_1_gd_id<0 or operand_1_gd_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid global data variable ID")
        operand_types=oppproto.OPERAND_TYPE_GLOBAL_DATA_VAR<<14
        operand_1=operand_1_gd_id
    elif operand_1_hf_id!=None:
        if operand_1_hf_id<0 or operand_1_hf_id>oppproto.MAX_HEADER_FIELDS-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid header field ID")
        operand_types=oppproto.OPERAND_TYPE_HEADER_FIELD<<14
        operand_1=operand_1_hf_id

    if operand_2_fd_id!=None:
        if operand_2_fd_id<0 or operand_2_fd_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid flow data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_FLOW_DATA_VAR<<12
        operand_2=operand_2_fd_id
    elif operand_2_gd_id!=None:
        if operand_2_gd_id<0 or operand_2_gd_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid global data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_GLOBAL_DATA_VAR<<12
        operand_2=operand_2_gd_id
    elif operand_2_hf_id!=None:
        if operand_2_hf_id<0 or operand_2_hf_id>oppproto.MAX_HEADER_FIELDS-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid header field ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_HEADER_FIELD<<12
        ...
        operand_types=operand_types | oppproto.OPERAND_TYPE_CONSTANT<<12
        operand_2=operand_2_cost

    if operand_3_fd_id!=None:
        if operand_3_fd_id<0 or operand_3_fd_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid flow data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_FLOW_DATA_VAR<<10
        operand_3=operand_3_fd_id
    elif operand_3_gd_id!=None:
        if operand_3_gd_id<0 or operand_3_gd_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid global data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_GLOBAL_DATA_VAR<<10
        operand_3=operand_3_gd_id
    elif operand_3_hf_id!=None:
        if operand_3_hf_id<0 or operand_3_hf_id>oppproto.MAX_HEADER_FIELDS-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid header field ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_HEADER_FIELD<<10
        operand_3=operand_3_hf_id
    else:
        operand_3 = 0

    if operand_4_fd_id!=None:
        if operand_4_fd_id<0 or operand_4_fd_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid flow data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_FLOW_DATA_VAR<<8
        operand_4=operand_4_fd_id
    elif operand_4_gd_id!=None:
        if operand_4_gd_id<0 or operand_4_gd_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid global data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_GLOBAL_DATA_VAR<<8
        operand_4=operand_4_gd_id
    elif operand_4_hf_id!=None:
        if operand_4_hf_id<0 or operand_4_hf_id>oppproto.MAX_HEADER_FIELDS-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid header field ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_HEADER_FIELD<<8
    ...
    if output_fd_id!=None:
        if output_fd_id<0 or output_fd_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid flow data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_FLOW_DATA_VAR<<7
        output=output_fd_id
    elif output_gd_id!=None:
        if output_gd_id<0 or output_gd_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionSetDataVariable: invalid global data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_GLOBAL_DATA_VAR<<7
        output=output_gd_id
    
    data=struct.pack(oppproto.OFP_EXP_ACTION_SET_DATA_VARIABLE_PACK_STR, act_type, operand_types, table_id, opcode, output, operand_1, operand_2, operand_3, operand_4, coeff_1, coeff_2, coeff_3, coeff_4, field_count, bit)
...
def OFPExpActionWriteContextToField(src_type,dst_field,src_id=None):
    ...
    ================ ======================================================
    """
    act_type=oppproto.OFPAT_EXP_WRITE_CONTEXT_TO_FIELD

    if src_type<oppproto.SOURCE_TYPE_FLOW_DATA_VAR or src_type>oppproto.SOURCE_TYPE_STATE:
        LOG.debug("OFPExpActionWriteContextToField: invalid src_type")

    if src_type in [oppproto.SOURCE_TYPE_FLOW_DATA_VAR,oppproto.SOURCE_TYPE_GLOBAL_DATA_VAR] and src_id==None:
        LOG.debug("OFPExpActionWriteContextToField: src_id cannot be None with \"flow_data_var\" or \"global_data_var\" type")

    if src_type==oppproto.SOURCE_TYPE_FLOW_DATA_VAR:
        if src_id<0 or src_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionWriteContextToField: invalid flow data variable ID")
    elif src_type==oppproto.SOURCE_TYPE_GLOBAL_DATA_VAR:
        if src_id<0 or src_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpActionWriteContextToField: invalid global data variable ID")
    elif src_type==oppproto.SOURCE_TYPE_STATE:
        src_id=0

    data=struct.pack(oppproto.OFP_EXP_WRITE_CONTEXT_TO_FIELD_PACK_STR, act_type, src_type, src_id, dst_field)
    return ofproto_parser.OFPActionExperimenterUnknown(experimenter=0xBEBABEBA, data=data)

def OFPExpMsgConfigureStatefulTable(datapath, stateful, table_id):
    command=oppproto.OFPSC_EXP_STATEFUL_TABLE_CONFIG
    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_STATEFUL_TABLE_CONFIG_PACK_STR,table_id,stateful)
    
    exp_type=oppproto.OFPT_EXP_STATE_MOD
...
def OFPExpMsgKeyExtract(datapath, command, fields, table_id, biflow=0, bit=0):
    ...
    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command) # msg type
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_EXTRACTOR_PACK_STR,table_id,biflow,bit,field_count)
    field_extract_format='!I'
...
def OFPExpMsgSetCondition(datapath, table_id, condition_id, condition, operand_1_fd_id=None, operand_1_gd_id=None, operand_1_hf_id=None, operand_2_fd_id=None, operand_2_gd_id=None, operand_2_hf_id=None):
    command=oppproto.OFPSC_EXP_SET_CONDITION

    if condition_id<0 or condition_id>oppproto.MAX_CONDITIONS_NUM-1:
    ...
    if operand_1_fd_id!=None:
        ...
        operand_types=oppproto.OPERAND_TYPE_FLOW_DATA_VAR<<6
        operand_1=operand_1_fd_id
    elif operand_1_gd_id!=None:
        if operand_1_gd_id<0 or operand_1_gd_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpMsgSetCondition: invalid global data variable ID")
        operand_types=oppproto.OPERAND_TYPE_GLOBAL_DATA_VAR<<6
        operand_1=operand_1_gd_id
    elif operand_1_hf_id!=None:
        if operand_1_hf_id<0 or operand_1_hf_id>oppproto.MAX_HEADER_FIELDS-1:
            LOG.debug("OFPExpMsgSetCondition: invalid header field ID")
        operand_types=oppproto.OPERAND_TYPE_HEADER_FIELD<<6
        operand_1=operand_1_hf_id

    if operand_2_fd_id!=None:
        if operand_2_fd_id<0 or operand_2_fd_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
            LOG.debug("OFPExpMsgSetCondition: invalid flow data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_FLOW_DATA_VAR<<4
        operand_2=operand_2_fd_id
    elif operand_2_gd_id!=None:
        if operand_2_gd_id<0 or operand_2_gd_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
            LOG.debug("OFPExpMsgSetCondition: invalid global data variable ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_GLOBAL_DATA_VAR<<4
        operand_2=operand_2_gd_id
    elif operand_2_hf_id!=None:
        if operand_2_hf_id<0 or operand_2_hf_id>oppproto.MAX_HEADER_FIELDS-1:
            LOG.debug("OFPExpMsgSetCondition: invalid header field ID")
        operand_types=operand_types | oppproto.OPERAND_TYPE_HEADER_FIELD<<4
        operand_2=operand_2_hf_id    

    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_SET_CONDITION_PACK_STR,table_id,condition_id,condition,operand_types,operand_1,operand_2)
    
    exp_type=oppproto.OFPT_EXP_STATE_MOD
    return ofproto_parser.OFPExperimenter(datapath=datapath, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)

def OFPExpMsgsSetGlobalDataVariable(datapath, table_id, global_data_variable_id, value, mask=0xffffffff):
    command=oppproto.OFPSC_EXP_SET_GLOBAL_DATA_VAR

    if global_data_variable_id<0 or global_data_variable_id>oppproto.MAX_GLOBAL_DATA_VAR_NUM-1:
        LOG.debug("OFPExpMsgsSetGlobalDataVariable: invalid global_data_variable_id")

    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_SET_GLOBAL_DATA_VAR_PACK_STR,table_id,global_data_variable_id,value,mask)
    
    exp_type=oppproto.OFPT_EXP_STATE_MOD
    return ofproto_parser.OFPExperimenter(datapath=datapath, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)

def OFPExpMsgSetFlowDataVariable(datapath, table_id, flow_data_variable_id, keys, value, mask=0xffffffff):
    command=oppproto.OFPSC_EXP_SET_FLOW_DATA_VAR

    key_count=len(keys)

    if flow_data_variable_id<0 or flow_data_variable_id>oppproto.MAX_FLOW_DATA_VAR_NUM-1:
        LOG.debug("OFPExpMsgSetFlowDataVariable: invalid flow_data_variable_id")

    if key_count > oppproto.MAX_KEY_LEN:
        key_count = 0
        LOG.debug("OFPExpMsgSetFlowDataVariable: Number of keys given > MAX_KEY_LEN")

    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_SET_FLOW_DATA_VAR_PACK_STR, table_id, flow_data_variable_id, key_count, value, mask)
...
def OFPExpMsgHeaderFieldExtract(datapath, table_id, extractor_id, field):
    command=oppproto.OFPSC_EXP_SET_HEADER_FIELD_EXTRACTOR

    if extractor_id<0 or extractor_id>oppproto.MAX_HEADER_FIELDS-1:
        LOG.debug("OFPExpMsgHeaderFieldExtract: invalid extractor_id")

    # TODO Davide: check if OXM_LENGTH(field)<=32 bit

    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_SET_HEADER_EXTRACTOR_PACK_STR,table_id,extractor_id,field)
...
def OFPExpMsgSetFlowState(datapath, state, keys, table_id, idle_timeout=0, idle_rollback=0, hard_timeout=0, hard_rollback=0, state_mask=0xffffffff):
    ...
    command=oppproto.OFPSC_EXP_SET_FLOW_STATE
    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_SET_FLOW_STATE_PACK_STR, table_id, key_count, state, state_mask, hard_rollback, idle_rollback, hard_timeout*1000, idle_timeout*1000)
    field_extract_format='!B'
...
def OFPExpMsgDelFlowState(datapath, keys, table_id):
    ...
    command=oppproto.OFPSC_EXP_DEL_FLOW_STATE
    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_DEL_FLOW_STATE_PACK_STR,table_id,key_count)
    field_extract_format='!B'
...
def OFPExpSetGlobalState(datapath, global_state, global_state_mask=0xffffffff):
    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, oppproto.OFPSC_EXP_SET_GLOBAL_STATE)
    data+=struct.pack(oppproto.OFP_EXP_STATE_MOD_SET_GLOBAL_STATE_PACK_STR,global_state,global_state_mask)
    
    exp_type=oppproto.OFPT_EXP_STATE_MOD
    return ofproto_parser.OFPExperimenter(datapath=datapath, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)

def OFPExpResetGlobalState(datapath):
    data=struct.pack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, oppproto.OFPSC_EXP_RESET_GLOBAL_STATE)
...
def OFPExpStateStatsMultipartRequest(datapath, table_id=ofproto.OFPTT_ALL, state=None, match=None):
    ...
    data=bytearray()
    msg_pack_into(oppproto.OFP_STATE_STATS_REQUEST_0_PACK_STR, data, 0, table_id, get_from_state, state)
    
    offset=oppproto.OFP_STATE_STATS_REQUEST_0_SIZE
    match.serialize(data, offset)

    exp_type=oppproto.OFPMP_EXP_STATE_STATS
...
def OFPExpStateStatsMultipartRequestAndDelete(datapath, table_id=ofproto.OFPTT_ALL, state=None, match=None):
    ...
    exp_type=oppproto.OFPMP_EXP_STATE_STATS_AND_DELETE
    return ofproto_parser.OFPExperimenterStatsRequest(datapath=datapath, flags=flags, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)
...
def OFPExpGlobalStateStatsMultipartRequest(datapath):
    flags = 0 # Zero or ``OFPMPF_REQ_MORE``    
    data=bytearray()
    exp_type=oppproto.OFPMP_EXP_GLOBAL_STATE_STATS
    return ofproto_parser.OFPExperimenterStatsRequest(datapath=datapath, flags=flags, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)
...
def OFPErrorExperimenterMsg_handler(ev):
    ...
    LOG.debug(' |-- exp_type: %s',oppproto.ofp_error_code_to_str(msg.type,msg.exp_type))
...
def OFPExpMsgAddPktTmp(datapath, pkttmp_id, pkt_data):
    command=oppproto.OFPSC_ADD_PKTTMP
    data=struct.pack(oppproto.OFP_EXP_PKTTMP_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_PKTTMP_MOD_ADD_PKTTMP_PACK_STR, pkttmp_id)
    data+=pkt_data
    #print("Data:%s\n" % ''.join( [ "%02X " % ord( x ) for x in data ] ).strip())
    
    exp_type=oppproto.OFPT_EXP_PKTTMP_MOD
    return ofproto_parser.OFPExperimenter(datapath=datapath, experimenter=0xBEBABEBA, exp_type=exp_type, data=data)

def OFPExpMsgDelPktTmp(datapath, pkttmp_id):
    command=oppproto.OFPSC_DEL_PKTTMP
    data=struct.pack(oppproto.OFP_EXP_PKTTMP_MOD_PACK_STR, command)
    data+=struct.pack(oppproto.OFP_EXP_PKTTMP_MOD_DEL_PKTTMP_PACK_STR, pkttmp_id)
...
class OFPInstructionInSwitchPktGen(ofproto_parser.OFPInstruction):
    ...
    def __init__(self, pkttmp_id, actions=None, len_=None):
        ...
        self.experimenter_id = oppproto.OPP_EXPERIMENTER_ID
        self.instr_type = oppproto.OFPIT_IN_SWITCH_PKT_GEN
    ...
    def parser(cls, buf, offset):
        (type_, len_, experimenter_id_, instr_type_) = struct.unpack_from(
            oppproto.OFP_INSTRUCTION_INSWITCH_PKT_GEN_PACK_STR,
            buf, offset)

        offset += oppproto.OFP_INSTRUCTION_INSWITCH_PKT_GEN_SIZE
        actions = []
        actions_len = len_ - oppproto.OFP_INSTRUCTION_INSWITCH_PKT_GEN_SIZE
        while actions_len > 0:
    ...
    def serialize(self, buf, offset):
        action_offset = offset + oppproto.OFP_INSTRUCTION_INSWITCH_PKT_GEN_SIZE
        if self.actions:
        ...
        msg_pack_into(oppproto.OFP_INSTRUCTION_INSWITCH_PKT_GEN_PACK_STR,
...
class OFPStateEntry(object):
    ...
    def parser(cls, buf, offset):
        ...
        entry.flow_data_var = []
        for f in range(oppproto.MAX_FLOW_DATA_VAR_NUM):
...
class OFPStateStats(StringifyMixin):
    ...
    def parser(cls, buf, offset=0):
        ...
        for i in range(len(buf)/oppproto.OFP_STATE_STATS_SIZE):
            state_stats = cls()
            ...
            (state_stats.length, state_stats.table_id, state_stats.dur_sec,
                ...
                oppproto.OFP_STATE_STATS_0_PACK_STR, buf, offset)
            offset += oppproto.OFP_STATE_STATS_0_SIZE

            state_stats.fields=[]
            field_extract_format='!I'
            if state_stats.field_count <= oppproto.MAX_FIELD_COUNT:
                for f in range(state_stats.field_count):
                    ...
                    offset +=4
            offset += ((oppproto.MAX_FIELD_COUNT-state_stats.field_count)*4)
            state_stats.entry = OFPStateEntry.parser(buf, offset)
            offset += oppproto.OFP_STATE_STATS_ENTRY_SIZE

            (state_stats.hard_rb, state_stats.idle_rb,state_stats.hard_to, state_stats.idle_to) = struct.unpack_from(
                oppproto.OFP_STATE_STATS_1_PACK_STR, buf, offset)
            offset += oppproto.OFP_STATE_STATS_1_SIZE
...
###############################################################################################################################
'''
[Opp to Open vSwitch wrapper]

An Opp application can be run by an Open vSwitch softswitch without Opp support by adding
'@Opp2OVSWrapper' just before def switch_features_handler(self, ev), after '@set_ev_cls()'

$ sudo mn --topo single,4 --mac --switch ovsk --controller remote    --->    starts Open vSwitch (OpenFlow only)
$ sudo mn --topo single,4 --mac --switch user --controller remote    --->    starts ofsoftswitch13 (OpenFlow + Opp)
$ ryu-manager [app_name]


How stuff works:
we need to intercept any message containing Opp code (Opp messages, FlowMod matching on 'state', FlowMod containing
SetState action, ...). The best way is intercepting any call to send_msg() in order to filter/modify/drop the outgoing message.
We don't want to touch Ryu code and we'd like minimal modifications to Opp applications code.
...
'''
OVSDatapath class inherits Datapath class to override send_msg() method in order to intercept and adapt Opp messages.
...
class OVSDatapath(Datapath):
    def send_msg(self, msg):
        '''
        In Opp a stateful stage has a state table and a flow table with the same table_id.
        ...
        if isinstance(msg,ofproto_parser.OFPExperimenter) and msg.experimenter==0xBEBABEBA and msg.exp_type==oppproto.OFPT_EXP_STATE_MOD:
            ...
            command_offset = struct.calcsize(oppproto.OFP_EXP_STATE_MOD_PACK_STR)
            (command,) = struct.unpack(oppproto.OFP_EXP_STATE_MOD_PACK_STR, msg.data[:command_offset])
            if command == oppproto.OFPSC_EXP_STATEFUL_TABLE_CONFIG:
                (table_id,stateful) = struct.unpack(oppproto.OFP_EXP_STATE_MOD_STATEFUL_TABLE_CONFIG_PACK_STR, msg.data[command_offset:command_offset+struct.calcsize(oppproto.OFP_EXP_STATE_MOD_STATEFUL_TABLE_CONFIG_PACK_STR)])
                ...
                return
            elif command == oppproto.OFPSC_EXP_SET_L_EXTRACTOR or command == oppproto.OFPSC_EXP_SET_U_EXTRACTOR:
                (table_id,field_count) = struct.unpack(oppproto.OFP_EXP_STATE_MOD_EXTRACTOR_PACK_STR,msg.data[command_offset:command_offset+struct.calcsize(oppproto.OFP_EXP_STATE_MOD_EXTRACTOR_PACK_STR)])
                if field_count>0:
                    fields_offset = command_offset+struct.calcsize(oppproto.OFP_EXP_STATE_MOD_EXTRACTOR_PACK_STR)
                    field_size = struct.calcsize('!I')
                    if command == oppproto.OFPSC_EXP_SET_L_EXTRACTOR:
                    ...
                    elif command == oppproto.OFPSC_EXP_SET_U_EXTRACTOR:
                ...
                return
            elif command == oppproto.OFPSC_EXP_SET_FLOW_STATE:
                ...
                '''
                (table_id, key_count, state, state_mask, hard_rollback, idle_rollback, hard_timeout, idle_timeout) = struct.unpack(oppproto.OFP_EXP_STATE_MOD_SET_FLOW_STATE_PACK_STR, msg.data[command_offset:command_offset+struct.calcsize(oppproto.OFP_EXP_STATE_MOD_SET_FLOW_STATE_PACK_STR)])
                if key_count>0:
                    lookup_fields = {}
                    key_starting_offset = command_offset+struct.calcsize(oppproto.OFP_EXP_STATE_MOD_SET_FLOW_STATE_PACK_STR)
                    key_offset = 0
                ...
                return
            elif command == oppproto.OFPSC_EXP_DEL_FLOW_STATE:
                # We should send a FlowMod DELETE by converting 'keys' into FlowMod's match fields
                (table_id, key_count) = struct.unpack(oppproto.OFP_EXP_STATE_MOD_DEL_FLOW_STATE_PACK_STR, msg.data[command_offset:command_offset+struct.calcsize(oppproto.OFP_EXP_STATE_MOD_DEL_FLOW_STATE_PACK_STR)])
                if key_count>0:
                    lookup_fields = {}
                    key_starting_offset = command_offset+struct.calcsize(oppproto.OFP_EXP_STATE_MOD_DEL_FLOW_STATE_PACK_STR)
                    key_offset = 0
                    # Parse the remaning payload to get the matching values of all lookup fields
                    for f in self.lookup_scope[table_id]:
                        field_bytes_num = ofproto_parser.OFPMatchField._FIELDS_HEADERS[f](f,0,0).oxm_len()
                    ...
                    LOG.debug("ERROR: empty key in STATE_MOD message!")
                return
            elif command == oppproto.OFPSC_EXP_SET_GLOBAL_STATE:
                ...
                return
            elif command == oppproto.OFPSC_EXP_RESET_GLOBAL_STATE:
                ...
                if isinstance(instr, ofproto_parser.OFPInstructionActions) and instr.type in [ofproto.OFPIT_WRITE_ACTIONS , ofproto.OFPIT_APPLY_ACTIONS]:
                    ...
                    for act in instr.actions:
                        if isinstance(act, ofproto_parser.OFPActionExperimenterUnknown) and act.experimenter == 0xBEBABEBA:
                            ...
                            if act_type == oppproto.OFPAT_EXP_SET_STATE:
                                (act_type, state, state_mask, table_id, hard_rollback, idle_rollback, hard_timeout, idle_timeout) = struct.unpack(oppproto.OFP_EXP_ACTION_SET_STATE_PACK_STR, act.data[:struct.calcsize(oppproto.OFP_EXP_ACTION_SET_STATE_PACK_STR)])
            ...
            # [step 2 ] check for he presence of any Opp match
            new_match_fields = []
...
'''
By decorating 'switch_features_handler()' with '@Opp2OVSWrapper', before the execution of the user-defined switch_features_handler()
