...
LOG = logging.getLogger('app.opp.ddos')

class OppDDoS(app_manager.RyuApp):

	def __init__(self, *args, **kwargs):
		super(OppDDoS, self).__init__(*args, **kwargs)
	...
	def switch_features_handler(self, event):
		...
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=0, idle_timeout=30)]
		...
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=1, idle_timeout=30)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions)]
		mod = ofparser.OFPFlowMod(datapath=datapath, table_id=1,
								priority=10, match=match, instructions=inst)
		datapath.send_msg(mod)
