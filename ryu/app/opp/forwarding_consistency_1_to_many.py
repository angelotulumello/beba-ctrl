...
$ ryu-manager ryu/ryu/app/opp/forwarding_consistency_1_to_many.py
$ sudo mn --topo single,4 --switch user --mac --controller remote
mininet> xterm h1 h1 h1 h2 h3 h4
h2# python ryu/ryu/app/opp/echo_server.py 200
h3# python ryu/ryu/app/opp/echo_server.py 300
h4# python ryu/ryu/app/opp/echo_server.py 400
...
LOG = logging.getLogger('app.opp.forwarding_consistency_1_to_many')

SWITCH_PORTS = 4
LOG.info("Opp Forwarding Consistency sample app initialized")
LOG.info("Supporting MAX %d ports per switch" % SWITCH_PORTS)

class OppLoadBalancing(app_manager.RyuApp):

    def __init__(self, *args, **kwargs):
        
        super(OppLoadBalancing, self).__init__(*args, **kwargs)
        self.mac_to_port = {}
    ...
    def switch_features_handler(self, ev):
        ...
        req = oppparser.OFPExpMsgConfigureStatefulTable(datapath=datapath, 
                                                    table_id=0, 
                                                    stateful=1)
        ...
        for port in range(2,SWITCH_PORTS+1):
            ...
            dest_tcp=(port)*100
            actions = [ oppparser.OFPExpActionSetState(state=port, table_id=0),
