...
class OpenStateEvolution(app_manager.RyuApp):
	...
	def install_leaves(self, datapath): 
		...
		actions_true = [oppparser.OFPExpActionSetState(state=1, table_id=3, idle_timeout=RTT), 
						ofparser.OFPActionOutput(1)]
						
		#if port 2 is better, set_state(2) and output 2
		actions_false = [oppparser.OFPExpActionSetState(state=2, table_id=3, idle_timeout=RTT), 
