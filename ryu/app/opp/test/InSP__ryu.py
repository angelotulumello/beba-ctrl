...
LOG = logging.getLogger('app.opp.pkttmp')

class OppInSP(app_manager.RyuApp):

	def __init__(self, *args, **kwargs):
		super(OppInSP, self).__init__(*args, **kwargs)
		if os.geteuid() != 0:
	...
	def add_pktgen_flow(self, datapath, table_id, priority, match, pkttmp_id, actions):
		if len(actions) > 0:
			inst = [oppparser.OFPInstructionInSwitchPktGen(pkttmp_id, actions)]
		else:
			inst = []
		mod = ofparser.OFPFlowMod(datapath=datapath, table_id=table_id,
								priority=priority, match=match, instructions=inst)
	...
	def test0(self,datapath):
		...
		req = oppparser.OFPExpMsgAddPktTmp(datapath=datapath, pkttmp_id=0, pkt_data=pkt_data)
