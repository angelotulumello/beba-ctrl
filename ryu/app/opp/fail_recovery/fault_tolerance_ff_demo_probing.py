...
LOG = logging.getLogger('app.opp.fault_tolerance_ff')

class OppFaultTolerance(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto.OFP_VERSION]
    
    def __init__(self, *args, **kwargs):
        super(OppFaultTolerance, self).__init__(*args, **kwargs)
    ...
    def install_probing(self, datapath):
        ...
        if datapath.id==2:
            ...
            actions = [oppparser.OFPExpActionSetState(state=17, table_id=0, hard_timeout=10, hard_rollback=100),
            ...
            actions = [oppparser.OFPExpActionSetState(state=0, table_id=0)]
            self.add_flow(datapath=datapath, table_id=0, priority=10,
                    match=match, actions=actions)
    ...
    def add_state_entry(self, datapath, mac_src, mac_dst):
        state = oppparser.OFPExpMsgSetFlowState(
