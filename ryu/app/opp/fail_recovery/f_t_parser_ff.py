'''
f_t_parser.py               -->     Opp version
f_t_parser_m_f.py           -->     Opp version with multiple faults
...
f_t_parser_ctrl_flags.py    -->     OpenFlow version: detection using Opp's States (instead of FF). The same as f_t_parser_ctrl_drop.py: in case local detour is not available, packets are sent to the CTRL. The CTRL installs the new rules but does not reroute received packets (it drops them).
f_t_parser_ff.py            -->     Opp version exploiting OF's Fast Failover instead of Opp's Global States
'''
    ...
    for i in range(len(requests)):
        ...
        for y in range(len(requests[request]['faults'].items())):
            ...
            if redirect_node == detect_node:
                ...
                max_len = 2000
                actions = [oppparser.OFPExpActionSetState(state=tag, table_id=0),ofparser.OFPActionSetField(mpls_label=tag),
                    ofparser.OFPActionOutput(mn_topo_ports['s'+str(redirect_node)]['s'+str(detour[1])],0)]
                weight = 0
                ...
                flow_entry['match']=ofparser.OFPMatch(in_port=mn_topo_ports['s'+str(redirect_node)]['s'+str(primary_path[node_index_in_p_p+1])],
                        eth_src=int_to_mac_str(request[0]),eth_dst=int_to_mac_str(request[1]),
                        eth_type=0x8847, mpls_label=tag)
                flow_entry['actions']=[oppparser.OFPExpActionSetState(state=tag, table_id=0),
                    ofparser.OFPActionOutput(mn_topo_ports['s'+str(redirect_node)]['s'+str(detour[1])],0)]
                flow_entry['inst']=[ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, flow_entry['actions'])]
                flow_entry['table_id']=0
                flow_entries_dict = add_flow_entry(flow_entries_dict,redirect_node,flow_entry)
