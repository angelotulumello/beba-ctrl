...
import ryu.ofproto.opp_v1_0  as oppproto 		# LUCA da sistemare cosi e brutto
...
########################### SET GD DATA VARIABLE ############################################
		...
		# actions = [oppparser.OFPExpActionSetState(state=1, table_id=0),
		# 			oppparser.OFPExpActionSetDataVariable(table_id=0, port_id=1, opcode=oppproto.OPCODE_SUM, output_mem_pd_id=0, operand_1_mem_pd_id=1, operand_2_mem_pd_id=1),
		# 			# oppparser.OFPExpActionWriteContextToField(src_type=oppproto.SOURCE_TYPE_GLOBAL_DATA_VAR,src_id=0,dst_field=ofproto.OXM_OF_MPLS_LABEL),
		# 			oppparser.OFPExpActionSetDataVariable(table_id=0, opcode=oppproto.OPCODE_SUM, output_gd_id=3, operand_1_hf_id=0, operand_2_cost=0),
		# 			ofparser.OFPActionOutput(ofproto.OFPP_FLOOD)]
		# inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions)]
		# mod = ofparser.OFPFlowMod(datapath=datapath, table_id=0,
		# 						priority=0, match=match, instructions=inst)
		...
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=0),
					# oppparser.OFPExpActionSetDataVariable(table_id=0, opcode=oppproto.OPCODE_SUM, output_fd_id=1, operand_1_hf_id=0, operand_2_cost=3)]
					# oppparser.OFPExpActionSetDataVariable(table_id=0, opcode=oppproto.OPCODE_SUM, output_fd_id=2, operand_1_gd_id=0, operand_2_gd_id=2)]
					oppparser.OFPExpActionSetDataVariable(table_id=0, opcode=oppproto.OPCODE_SUM, output_fd_id=0, operand_1_gd_id=2, operand_2_cost=0),
		...
		actions = [oppparser.OFPExpActionSetState(state=2, table_id=1),
					oppparser.OFPExpActionSetDataVariable(table_id=1, opcode=oppproto.OPCODE_SUM, output_fd_id=1, operand_1_hf_id=0, operand_2_cost=1),
					ofparser.OFPActionOutput(ofproto.OFPP_FLOOD)]
					# oppparser.OFPExpActionSetDataVariable(table_id=1, opcode=oppproto.OPCODE_SUM, output_gd_id=0, operand_1_gd_id=1, operand_2_cost=3)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions)]
		...
		# actions = [oppparser.OFPExpActionSetState(state=2, table_id=1),
		# 			oppparser.OFPExpActionSetDataVariable(table_id=1, opcode=oppproto.OPCODE_SUM, output_gd_id=0, operand_1_gd_id=3, operand_2_cost=3)]
		# inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions)]
		# mod = ofparser.OFPFlowMod(datapath=datapath, table_id=1,
		# 						priority=0, match=match, instructions=inst)
		# datapath.send_msg(mod)
		...
		# 		else:
		# 			out_port = s
		# 		actions = [oppparser.OFPExpActionSetState(state=i, table_id=0, hard_timeout=10),
