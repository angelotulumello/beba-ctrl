...
LOG = logging.getLogger('app.opp.maclearning.state_sync')
...
class OSMacLearning(app_manager.RyuApp):
	...
	def switch_features_handler(self, event):
		...
		""" Switche sent his features, check if Opp supported """
		msg = event.msg
		...
		for i in range(1, N+1):
			for s in range(N+1):
				...
				if s == 0:
					...
					out_port = s
				actions = [
					oppparser.OFPExpActionSetState(state=i, table_id=0, hard_timeout=10),
	...
	def packet_in_handler(self, event):
		...
		if (msg.body.experimenter == 0xBEBABEBA):
			if(msg.body.exp_type == oppproto.OFPMP_EXP_STATE_STATS):
				state_entry_list = oppparser.OFPStateStats.parser(msg.body.data)
				if (state_entry_list==[]):
					print "No key for this state"
				else:
					for state_entry in state_entry_list:
						print 'State :',state_entry.entry.state
						print 'Key   :',oppparser.state_entry_key_to_str(state_entry)
						print '*********'
...
def ask_for_state(t, k, state):
	...
	while counter < k:
		...
		if devices==[]:
			...
			m = oppparser.OFPExpGetFlowsInState(devices[0], table_id=0, state=state)
