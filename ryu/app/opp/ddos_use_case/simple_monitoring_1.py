...
LOG = logging.getLogger('app.opp.simple_monitoring_1')
timewindow = 4
precision = 3  # 1 -> small precision 68% / 2 -> medium precision 95% / 3 -> high precision 99,7%


class SimpleMonitoring_1(selective_monitoring_1.OppSelectiveMonitoring_1):
    ...
    def _request_stats(self, datapath):
        for table in range(6):
            req = oppparser.OFPExpStateStatsMultipartRequestAndDelete(datapath, table_id=table)
    ...
    def printcounters(self):
        ...
        """ LOG.info('Length=%s Table ID=%s Duration_sec=%s Duration_nsec=%s Field_count=%s\n'
            ...
            oppparser.state_entry_key_to_str(state_stats_list[index].fields, state_stats_list[index].entry.key, state_stats_list[index].entry.key_count), str(state_stats_list[index].entry.state),
    ...
    def _state_stats_reply_handler(self, ev):
        ...
        if (msg.body.experimenter == 0XBEBABEBA):
            if (msg.body.exp_type == oppproto.OFPMP_EXP_STATE_STATS_AND_DELETE):
                data = msg.body.data
                state_stats_list = oppparser.OFPStateStats.parser(data, 0)
