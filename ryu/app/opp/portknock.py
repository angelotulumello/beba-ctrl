...
LOG = logging.getLogger('app.opp.portknock')
...
class OppPortKnocking(app_manager.RyuApp):

	def __init__(self, *args, **kwargs):
		super(OppPortKnocking, self).__init__(*args, **kwargs)
	...
	def switch_features_handler(self, ev):
		...
		for i in range(len(port_list)):
			...
			if port_list[i] != final_port and port_list[i] != second_last_port:
				...
				actions = [oppparser.OFPExpActionSetState(state=i+1, table_id=0, idle_timeout=5)]		
			elif port_list[i] == second_last_port:
				# In the transaction to the OPEN state, the timeout is set to 10 sec
				actions = [oppparser.OFPExpActionSetState(state=i+1, table_id=0, idle_timeout=10)]
			else:
