...
$ ryu-manager ~/opp-ctrl/ryu/app/opp/IPTablesCase1.py
...
Step-by-step Simulation Startup
1) Launch server load balancing controller application in Mininet by typing the following command:
$ ryu-manager ~/opp-ctrl/ryu/app/opp/IPTablesCase2.py
...
Step-by-step Simulation Startup
1) Launch server load balancing controller application in Mininet by typing the following command:
$ ryu-manager ~/opp-ctrl/ryu/app/opp/IPTablesCase3.py
...
class OpenStateEvolution(app_manager.RyuApp):
	...
	############################### LOOKUP/UPDATE ###################################
		...
		actions = [oppparser.OFPExpActionSetState(state=11, table_id=0)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions),
		...
		actions = [oppparser.OFPExpActionSetState(state=12, table_id=0)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions),
			ofparser.OFPInstructionWriteMetadata(metadata=0, metadata_mask=0xFFFFFFFF),
			ofparser.OFPInstructionGotoTable(4)]
		mod = ofparser.OFPFlowMod(datapath=datapath, table_id=0,
	...
	############################### LOOKUP/UPDATE ###################################
		...
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=0),
				   oppparser.OFPExpActionSetDataVariable(table_id=0, opcode=oppproto.OPCODE_SUM, output_gd_id=0, operand_1_gd_id=0, operand_2_cost=1),
		...
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=3),
				   ofparser.OFPActionSetField(ipv4_dst='10.0.0.2'),
				   ofparser.OFPActionSetField(tcp_dst=80),
				   # tolto per test
				   oppparser.OFPExpActionSetDataVariable(table_id=3, opcode=oppproto.OPCODE_SUM, output_fd_id=0, operand_1_gd_id=1, operand_2_cost=0),
				   oppparser.OFPExpActionSetDataVariable(table_id=3, opcode=oppproto.OPCODE_SUM, output_fd_id=1, operand_1_gd_id=0, operand_2_cost=80)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions),
			ofparser.OFPInstructionGotoTable(4)]
		...
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=3),
				   ...
				   ofparser.OFPActionSetField(tcp_dst=80),
				   # tolto per test
				   oppparser.OFPExpActionSetDataVariable(table_id=3, opcode=oppproto.OPCODE_SUM, output_fd_id=0, operand_1_gd_id=2, operand_2_cost=0),
				   oppparser.OFPExpActionSetDataVariable(table_id=3, opcode=oppproto.OPCODE_SUM, output_fd_id=1, operand_1_gd_id=0, operand_2_cost=80)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions),
			ofparser.OFPInstructionGotoTable(4)]
		mod = ofparser.OFPFlowMod(datapath=datapath, table_id=3,
		...
		actions = [oppparser.OFPExpActionWriteContextToField(src_type=oppproto.SOURCE_TYPE_FLOW_DATA_VAR,src_id=0,dst_field=ofproto.OXM_OF_IPV4_DST),
	...
	############################### LOOKUP/UPDATE ###################################
		...
		for stateVal in range(1,21):
			state = oppparser.OFPExpMsgSetFlowState(datapath=datapath,
					state=2000+stateVal,
		...
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=0),
				   ...
				   oppparser.OFPExpActionWriteContextToField(src_type=oppproto.SOURCE_TYPE_GLOBAL_DATA_VAR,src_id=1,dst_field=ofproto.OXM_OF_METADATA)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions),
		...
		match = ofparser.OFPMatch(state=0, in_port=LAN_PORT, eth_type=0x0800)
		actions = [oppparser.OFPExpActionSetState(state=1, table_id=3),
				   oppparser.OFPExpActionSetDataVariable(table_id=3, opcode=oppproto.OPCODE_SUM, output_fd_id=1, operand_1_hf_id=0, operand_2_cost=0),
				   ofparser.OFPActionSetField(ipv4_src="1.0.0.1"),
				   oppparser.OFPExpActionWriteContextToField(src_type=oppproto.SOURCE_TYPE_FLOW_DATA_VAR,src_id=1,dst_field=ofproto.OXM_OF_TCP_SRC)]
		inst = [ofparser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS,actions),
			ofparser.OFPInstructionGotoTable(4)]
		mod = ofparser.OFPFlowMod(datapath=datapath, table_id=3,
								priority=97, match=match, instructions=inst)
